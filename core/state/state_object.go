...
func (so *stateObject) GetCommittedState(key common.Hash) common.Hash {
	...
	enc, err := so.db.stateReader.ReadAccountStorage(so.address,accounts.GetIncarnation(so.address), &key)
...
func (so *stateObject) updateTrie(stateWriter StateWriter) error {
	for key, value := range so.dirtyStorage {
		...
		so.originStorage[key] = value
		fmt.Println("core/state/state_object.go:211 updateTrie", so.address.String(), "version", accounts.GetIncarnation(so.address))
		fmt.Println("core/state/state_object.go:211 k=", key.String(), "v=", value.String())
		if err := stateWriter.WriteAccountStorage(so.address,accounts.GetIncarnation(so.address), &key, &original, &value); err != nil {
			return err
		}
	}
	return nil
